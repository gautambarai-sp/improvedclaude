...
# ===== PAGE 6: CUSTOMER ANALYTICS =====
    ...
    nat_dist = filtered['Nationality'].value_counts().head(10).reset_index()
    nat_dist.columns = ['Nationality', 'Count']
    
    chart = alt.Chart(nat_dist).mark_bar().encode(
    ...
    # Persona Insights
    st.subheader("🎯 Primary Customer Persona")
    
    top_age = filtered['AgeGroup'].value_counts().index[0]
    top_nation = filtered['Nationality'].value_counts().index[0]
    top_gender = filtered['Gender'].value_counts().index[0] if 'Gender' in filtered.columns else 'Unknown'
    
    persona = filtered[(filtered['AgeGroup'] == top_age) & (filtered['Nationality'] == top_nation)]
    if len(persona) > 0:
        top_dept_persona = persona['Department'].value_counts().index[0]
        top_store_persona = persona['Store_Format'].value_counts().index[0]
        persona_sales = persona['SalesAmount'].sum()
        persona_txns = persona['Transaction'].nunique()
        
        col1, col2, col3, col4 = st.columns(4)
        col1.metric("Age Group", top_age)
        col2.metric("Nationality", top_nation)
        col3.metric("Top Dept", top_dept_persona)
        col4.metric("Transactions", f"{persona_txns:,}")
        
        st.markdown(f"""
        <div class="insight-box">
        <b>💡 Primary Persona Insights:</b><br>
        • This segment represents {(len(persona)/len(filtered)*100):.1f}% of all customers<br>
        • Total spending: AED {persona_sales:,.0f}<br>
        • Preferred store: {top_store_persona}<br>
        • Most purchased: {top_dept_persona}
        </div>
        """, unsafe_allow_html=True)
    ...
    loyalty = filtered.groupby(['AgeGroup', 'Nationality']).agg(
    ...
    st.markdown(f"""
    <div class="recommendation-box">
    <b>✅ Customer Targeting Recommendations:</b><br>
    1. <b>Focus Marketing on {top_age}:</b> This is your largest segment<br>
    2. <b>Localize for {top_nation}:</b> Create culture-specific campaigns<br>
    3. <b>Loyalty Program:</b> Reward repeat customers with personalized offers<br>
    4. <b>Cross-sell {top_dept_persona}:</b> Bundle with complementary products<br>
    5. <b>Expected Impact:</b> 20-30% increase in customer retention
    </div>
    """, unsafe_allow_html=True)
    
    # Decision
    st.subheader("❓ Customer Strategy Decision")
    target_age = st.selectbox("Which age group to target?", sorted(filtered['AgeGroup'].unique()))
    target_nat = st.selectbox("Which nationality to focus on?", sorted(filtered['Nationality'].unique()))
    loyalty_program = st.checkbox("Launch loyalty program for this segment?")
    
    if st.button("💾 Save Customer Strategy"):
        st.success(f"✅ Customer targeting strategy saved! Focus: {target_age}, {target_nat}")

# ===== PAGE 7: ACTION PLAN =====
elif page == "🎯 Action Plan":
    st.title("🎯 Strategic Action Plan")
    st.write("*Executive summary with prioritized recommendations*")
    ...
    worst_campaign = filtered.groupby('Campaign')['SalesAmount'].sum().idxmin()
    best_age = filtered['AgeGroup'].value_counts().index[0]
    best_format = filtered.groupby('Store_Format')['SalesAmount'].sum().idxmax()
    
    best_city_sales = filtered[filtered['City'] == best_city]['SalesAmount'].sum()
    worst_city_sales = filtered[filtered['City'] == worst_city]['SalesAmount'].sum()
    gap = ((best_city_sales - worst_city_sales) / worst_city_sales * 100) if worst_city_sales > 0 else 0
    
    # Top 3 Recommendations
    st.subheader("🎯 Top 3 Strategic Priorities")
    
    st.markdown(f"""
    <div class="recommendation-box">
    <b>1️⃣ GEOGRAPHIC EXPANSION - Priority: HIGH</b><br>
    <b>Objective:</b> Replicate {best_city}'s success in {worst_city}<br>
    <b>Current Gap:</b> {gap:.0f}% performance difference<br>
    <b>Action Items:</b><br>
    • Apply {best_campaign} campaign in {worst_city}<br>
    • Increase {best_dept} inventory by 25%<br>
    • Target {best_age} demographic with localized messaging<br>
    <b>Investment:</b> AED 500,000<br>
    <b>Expected ROI:</b> 150-200% within 90 days<br>
    <b>Quick Win:</b> Start with best-performing department in underperforming city
    </div>
    """, unsafe_allow_html=True)
    
    st.markdown(f"""
    <div class="recommendation-box">
    <b>2️⃣ DEPARTMENT OPTIMIZATION - Priority: HIGH</b><br>
    <b>Objective:</b> Boost {worst_dept} performance by 30%<br>
    <b>Action Items:</b><br>
    • Apply {best_campaign} marketing strategy to {worst_dept}<br>
    • Increase promotional budget by 15%<br>
    • Cross-promote with {best_dept}<br>
    • Optimize shelf placement in {best_format} stores<br>
    <b>Investment:</b> AED 250,000<br>
    <b>Expected ROI:</b> 120-150% within 60 days<br>
    <b>Quick Win:</b> Bundle promotions with top-selling items
    </div>
    """, unsafe_allow_html=True)
    
    st.markdown(f"""
    <div class="recommendation-box">
    <b>3️⃣ CUSTOMER RETENTION - Priority: MEDIUM</b><br>
    <b>Objective:</b> Increase repeat purchase rate by 25%<br>
    <b>Action Items:</b><br>
    • Launch loyalty program for {best_age} age group<br>
    • Personalized offers based on purchase history<br>
    • Exclusive early access to new products<br>
    • Referral incentives<br>
    <b>Investment:</b> AED 150,000<br>
    <b>Expected ROI:</b> 180-220% within 6 months<br>
    <b>Quick Win:</b> Email campaign to top 20% customers with personalized discounts
    </div>
    """, unsafe_allow_html=True)
    ...
    roadmap_data = {
        'Phase': ['Week 1-2', 'Week 3-4', 'Month 2', 'Month 3', 'Month 4-6'],
        'Focus': [
            'Quick Wins',
        ...
        'Expected Impact': ['5-10%', '10-15%', '15-25%', '20-30%', '25-35%'],
        'Investment': ['AED 50K', 'AED 150K', 'AED 300K', 'AED 200K', 'AED 150K']
    }
    ...
    st.subheader("📊 Current Performance Scorecard")
    ...
    scorecard = pd.DataFrame({
        ...
        'Value': [
            ...
            best_age,
            best_format,
            f"AED {filtered['SalesAmount'].sum():,.0f}",
            f"AED {filtered.groupby('Transaction')['SalesAmount'].sum().mean():,.0f}"
        ],
        'Status': [
            '✅ Scale',
            '⚠️ Improve',
        ...
        'Action': [
            'Expand footprint',
            'Apply best practices from top city',
            'Increase inventory 20%',
            'Boost marketing 15%',
            'Increase budget 30-50%',
            'Redesign or discontinue',
            'Focus all campaigns here',
            'Open more of this format',
            'Set 25% YoY growth target',
            'Aim for 15% increase'
        ]
    })
    ...
    # Final Decisions
    st.subheader("❓ Final Strategic Decisions")
    
    col1, col2 = st.columns(2)
    
    with col1:
        decision1 = st.radio("Overall marketing budget adjustment:", [
            "Increase by 20%",
            "Increase by 50%",
            "Keep current level",
            "Redistribute existing budget"
        ])
        
        decision2 = st.radio("Priority investment area:", [
            "Geographic Expansion",
            "Department Growth",
        ...
        decision3 = st.radio("Implementation timeline:", [
            "Immediate (This Week)",
            "Short Term (1 Month)",
    ...
    with col2:
        decision4 = st.multiselect("Cities to expand:", 
            [c for c in df['City'].unique() if c != 'Unknown'])
        
        decision5 = st.multiselect("Campaigns to scale:", 
            [c for c in df['Campaign'].unique() if c != 'Unknown'])
        
        decision6 = st.text_area("Additional strategic notes:", 
            "Enter any additional considerations or constraints...")
    ...
    with col1:
        if st.button("💾 Save Action Plan"):
            st.success("✅ Strategic action plan saved successfully!")
    
    with col2:
        if st.button("📊 Generate Executive Report"):
            report_data = {
                'Decision': ['Budget', 'Priority', 'Timeline', 'Cities', 'Campaigns'],
                'Value': [
                    decision1,
                    decision2,
                    decision3,
                    ', '.join(decision4) if decision4 else 'None selected',
                    ', '.join(decision5) if decision5 else 'None selected'
                ]
            }
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"lulu_action_plan_{datetime.now().strftime('%Y%m%d')}.csv",
                mime="text/csv"
            )
    
    with col3:
        if st.button("📧 Share with Team"):
            st.info("📧 Action plan shared with stakeholders")
    ...
    st.markdown("""
    <div class="insight-box">
    <b>📈 Expected Overall Business Impact:</b><br>
    • <b>Revenue Growth:</b> 25-35% increase within 6 months<br>
    • <b>Customer Acquisition:</b> 15-20% more new customers<br>
    • <b>Customer Retention:</b> 20-30% improvement in repeat rate<br>
    • <b>Marketing ROI:</b> 150-200% average across all campaigns<br>
    • <b>Geographic Coverage:</b> Expand to 2-3 new high-potential areas<br>
    • <b>Market Share:</b> 5-8% increase in competitive positioning
    </div>
    """, unsafe_allow_html=True)
...
if st.sidebar.button("📥 Export Filtered Data"):
    ...
    st.sidebar.download_button(
        label="Download CSV",
        data=csv,
        file_name=f"lulu_export_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
        mime="text/csv"
    )

st.sidebar.divider()
st.sidebar.info(f"📊 **Data Summary**\n\n{len(filtered):,} rows | {filtered['SalesAmount'].sum():,.0f} AED")
